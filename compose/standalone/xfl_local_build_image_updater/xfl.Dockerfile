...
# RUN apk add curl yq-go jq python3 py3-pip
# RUN apk add curl wget jq python3 py3-pip
# RUN wget -O- https://get.docker.com/ | sh
# RUN mkdir /app
# # 不安装错误的 py 库
# RUN cat /app/requirements.txt | grep -v "docker_py" > /app/requirements.txt 
# COPY --chmod=755 app/requirements.txt /app/
